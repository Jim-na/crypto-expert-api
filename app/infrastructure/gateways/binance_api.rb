  ...
  module Binance
    ...
    class Api
      ...
      def spotpair_klines(symbol)
        HttpApi::Request.new(BASIC_URL,
                             nil).get("api/v3/klines?symbol=#{symbol}&interval=#{TIME_INTERVAL}&limit=#{LIMIT + 1}").parse
      end
      ...
      def futurepair_klines(symbol)
        HttpApi::Request.new(FUTURE_URL,
                             @token).get("fapi/v1/klines?symbol=#{symbol}&interval=#{TIME_INTERVAL}&limit=#{LIMIT + 1}").parse
      end

      def longshort_ratio(symbol)
        HttpApi::Request.new(FUTURE_URL,
                             @token).get("futures/data/globalLongShortAccountRatio?symbol=#{symbol}&period=#{TIME_INTERVAL}&limit=#{LIMIT}").parse
      end

      def open_interest(symbol)
        HttpApi::Request.new(FUTURE_URL,
                             @token).get("futures/data/openInterestHist?symbol=#{symbol}&period=#{TIME_INTERVAL}&limit=#{LIMIT}").parse
      end
